...
def test_convdraw_init():
    with pytest.raises(AssertionError):
        ConvDRAWLoss(None, 5, 3, 2.0)  # input cost scale out of bounds
...
def test_convdraw_loss():
    ...
    convdraw_loss = ConvDRAWLoss(fit_loss=lambda y, x: 1.0, input_dim=4, latent_dim=2, input_cost_scale=0.5)
...
def test_gradient():
    ...
    conv_draw_nn = ConvDRAW(enc_nn, dec_nn, num_steps=2, batch_size=batch_size, input_shape=input_shape,
                            latent_dim=latent_dim, encoder_output_shape=(2, 2, 2), rnn_hidden_channels=1,
    ...
    # loss function
    loss_fn = ConvDRAWLoss(SigmoidBinaryCrossEntropyLoss(from_sigmoid=False, batch_axis=0), input_dim, latent_dim)
